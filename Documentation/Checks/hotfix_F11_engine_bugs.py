...
Phase D: Smoke import both modules
Phase E: Restart astro service
Phase F: Smoke-test all 4 previously-broken endpoints
...
# ============================================================================
# Phase D: Import smoke
# ============================================================================

def phase_d_import():
    log("STEP", "Phase D — Import smoke test")
    test_code = """
import sys
sys.path.insert(0, '/opt/astro')
import remedies_esoteric
import karmic
# Verify the patched functions exist
assert callable(remedies_esoteric.vbt_breath_techniques)
assert callable(remedies_esoteric.vbt_awareness_techniques)
assert callable(remedies_esoteric.vbt_devotional_practices)
assert callable(karmic.handle_kaal_sarpa)
print('IMPORT_OK')
"""
    result = subprocess.run(
        [sys.executable, "-c", test_code],
        capture_output=True, text=True, cwd=ASTRO_DIR
    )
    if result.returncode != 0 or "IMPORT_OK" not in result.stdout:
        die(f"Import test failed:\nSTDOUT: {result.stdout}\nSTDERR: {result.stderr}")
    log("OK", "Modules import and patched functions are callable")


# ============================================================================
# Phase E: Restart service
# ============================================================================

def phase_e_restart():
    log("STEP", "Phase E — Restart astro service")
    result = subprocess.run(
...
# ============================================================================
# Phase F: Smoke-test the 4 previously-broken endpoints
# ============================================================================

def phase_f_smoke():
    log("STEP", "Phase F — Smoke-test 4 previously-broken endpoints")
